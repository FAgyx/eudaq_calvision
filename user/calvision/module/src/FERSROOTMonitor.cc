...
private:
  TH1D* m_FERS_LG_Ch_ADC[MAX_NBRD][FERSLIB_MAX_NCH_5202]{};
  TH1D* m_FERS_HG_Ch_ADC[MAX_NBRD][FERSLIB_MAX_NCH_5202]{};

  TProfile* m_DRS_Pulse_Ch[MAX_NBRD][MAX_X742_CHANNEL_SIZE*MAX_X742_GROUP_SIZE]{};  // 4gr x 9ch/gr
...
//  TH1I* m_FERS_hv_status_on;
  ...
  int brd = 0;
  int PID = 0;
  int m_num_fers_boards = 1;
  int m_num_drs_boards = 1;
...
void FERSROOTMonitor::AtConfiguration(){
        ...
        m_num_fers_boards = conf ? conf->Get("FERS_MONITOR_BOARD_COUNT", 1) : 1;
        if (m_num_fers_boards < 1) m_num_fers_boards = 1;
        if (m_num_fers_boards > MAX_NBRD) m_num_fers_boards = MAX_NBRD;
        ...
        m_num_drs_boards = conf ? conf->Get("DRS_MONITOR_BOARD_COUNT", shmp ? shmp->connectedboardsDRS : 1) : 1;
        if (m_num_drs_boards < 1) m_num_drs_boards = 1;
        if (m_num_drs_boards > MAX_NBRD) m_num_drs_boards = MAX_NBRD;

  m_FERS_tempFPGA =  m_monitor->Book<TH1D>("Monitor1/FERS_tempFPGA","FERS_tempFPGA","h_FERS_tempFPGA","FERS_tempFPGA;Board;tempFPGA [C]",m_num_fers_boards,-0.5,m_num_fers_boards-0.5);
  m_FERS_tempDetector =  m_monitor->Book<TH1D>("Monitor1/FERS_tempDetector","FERS_tempDetector","h_FERS_tempDetector","FERS_tempDetector;Board;tempDetector [C]",m_num_fers_boards,-0.5,m_num_fers_boards-0.5);
  m_FERS_hv_Vmon =  m_monitor->Book<TH1D>("Monitor1/FERS_hv_Vmon","FERS_hv_Vmon","h_FERS_hv_Vmon","FERS_hv_Vmon;Board;hv_Vmon [V]",m_num_fers_boards,-0.5,m_num_fers_boards-0.5);
  m_FERS_hv_Imon =  m_monitor->Book<TH1D>("Monitor1/FERS_hv_Imon","FERS_hv_Imon","h_FERS_hv_Imon","FERS_hv_Imon;Board;hv_Imon [mA]",m_num_fers_boards,-0.5,m_num_fers_boards-0.5);
  ...
  m_FERS_SUMadc  =m_monitor->Book<TH2F>("Monitor2/FERS_SUMadc", "FERS_SUMadc" ,"h_FERS_SUMadc","FERS: (Sum ADC)/evt;FERS channel;FERS brd x2", 64,0,64,m_num_fers_boards*2,0.,m_num_fers_boards*2);
...
*/
  ...
  for(int i=0;i<m_num_drs_boards;i++) {
        char hname[256];
	char tname[256];
	char sname[256];
							...
							if (Event.GrPresent[igr]) {
								for (int ich=0; ich<MAX_X742_CHANNEL_SIZE;ich++){
									...
									for(int its=0;its<Event.DataGroup[igr].ChSize[ich];its++){
										m_DRS_Pulse_Ch[brd][hch]->Fill(its,Event.DataGroup[igr].DataChannel[ich][its]);
									}
								}
							}
					}
