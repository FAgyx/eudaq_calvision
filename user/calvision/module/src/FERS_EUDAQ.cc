...
constexpr int kLegacySpectEventSize = static_cast<int>(sizeof(SpectEvent_t) - 2 * sizeof(uint64_t));
constexpr int kLegacyCountingEventSize = static_cast<int>(sizeof(CountingEvent_t) - sizeof(double));

SpectEvent_t UnpackSpectEventPayload(const std::vector<uint8_t> &data);
void PackSpectEventPayload(const SpectEvent_t &event, std::vector<uint8_t> *vec);
...
void FERSpack(int nbits, uint16_t input, std::vector<uint8_t> *vec)
{
	for (int i = 0; i < nbits / 8; ++i) {
		vec->push_back(static_cast<uint8_t>((input >> (8 * i)) & 0xFFu));
	}
}
...
void FERSpack(int nbits, uint64_t input, std::vector<uint8_t> *vec)
{
	for (int i = 0; i < nbits / 8; ++i) {
		vec->push_back(static_cast<uint8_t>((input >> (8 * i)) & 0xFFu));
	}
...
void FERSpack(int nbits, double input, std::vector<uint8_t> *vec)
{
	if (nbits != static_cast<int>(8 * sizeof(double))) {
		EUDAQ_THROW("FERSpack(double): unsupported width " + std::to_string(nbits));
	}

	uint64_t bits = 0;
	std::memcpy(&bits, &input, sizeof(bits));
	FERSpack(static_cast<int>(8 * sizeof(bits)), bits, vec);
}
...
double FERSunpackDouble(int index, const std::vector<uint8_t>& vec)
{
	uint64_t bits = FERSunpack64(index, vec);
	double out = 0.0;
...
void FERSpack_spectevent(void* Event, std::vector<uint8_t> *vec)
{
  ...
  PackSpectEventPayload(*tmpEvent, vec);
}
...
SpectEvent_t FERSunpack_spectevent(std::vector<uint8_t> *vec)
{
  ...
  return UnpackSpectEventPayload(data);
}
...
void FERSpack_countevent(void* Event, std::vector<uint8_t> *vec)
{
  ...
  FERSpack( 8*sizeof(double), tmpEvent->rel_tstamp_us,  vec);
...
CountingEvent_t FERSunpack_countevent(std::vector<uint8_t> *vec)
{
  ...
  if (dsize != static_cast<int>(sizeof(CountingEvent_t)) && dsize != kLegacyCountingEventSize) {
	  EUDAQ_THROW("*** WRONG DATA SIZE FOR COUNT EVENT! " + std::to_string(sizeof(CountingEvent_t)) +
		      " vs " + std::to_string(dsize) + " ***");
  }
  int index = 0;
  tmpEvent.tstamp_us = FERSunpackDouble(index, data); index += sizeof(double);
  if (dsize == static_cast<int>(sizeof(CountingEvent_t))) {
	  tmpEvent.rel_tstamp_us = FERSunpackDouble(index, data); index += sizeof(double);
  }
...
void PackSpectEventPayload(const SpectEvent_t &event, std::vector<uint8_t> *vec)
{
	constexpr int nchan = 64;
	FERSpack(static_cast<int>(8 * sizeof(double)), event.tstamp_us, vec);
	FERSpack(static_cast<int>(8 * sizeof(double)), event.rel_tstamp_us, vec);
	FERSpack(64, event.tstamp_clk, vec);
	FERSpack(64, event.Tref_tstamp, vec);
	FERSpack(64, event.trigger_id, vec);
	FERSpack(64, event.chmask, vec);
	FERSpack(64, event.qdmask, vec);
	for (int i = 0; i < nchan; ++i) FERSpack(16, event.energyHG[i], vec);
	for (int i = 0; i < nchan; ++i) FERSpack(16, event.energyLG[i], vec);
	for (int i = 0; i < nchan; ++i) FERSpack(32, event.tstamp[i], vec);
	for (int i = 0; i < nchan; ++i) FERSpack(16, event.ToT[i], vec);
}
...
SpectEvent_t UnpackSpectEventPayload(const std::vector<uint8_t> &data)
{
	...
	const int dsize = static_cast<int>(data.size());
	const int tsize = static_cast<int>(sizeof(SpectEvent_t));
	if (dsize != tsize && dsize != kLegacySpectEventSize) {
		EUDAQ_THROW("*** WRONG DATA SIZE FOR SPECT EVENT! " + std::to_string(tsize) +
			    " vs " + std::to_string(dsize) + " ***");
	}

	const bool has_extended_timestamps = (dsize == tsize);
	int index = 0;
	tmpEvent.tstamp_us = FERSunpackDouble(index, data); index += sizeof(double);
	tmpEvent.rel_tstamp_us = FERSunpackDouble(index, data); index += sizeof(double);
	if (has_extended_timestamps) {
		tmpEvent.tstamp_clk = FERSunpack64(index, data); index += 8;
		tmpEvent.Tref_tstamp = FERSunpack64(index, data); index += 8;
	}
	tmpEvent.trigger_id = FERSunpack64(index, data); index += 8;
	tmpEvent.chmask = FERSunpack64(index, data); index += 8;
	tmpEvent.qdmask = FERSunpack64(index, data); index += 8;
	for (int i = 0; i < nchan; ++i) {
		tmpEvent.energyHG[i] = FERSunpack16(index, data);
		index += 2;
	}
	for (int i = 0; i < nchan; ++i) {
		tmpEvent.energyLG[i] = FERSunpack16(index, data);
		index += 2;
	}
	for (int i = 0; i < nchan; ++i) {
		tmpEvent.tstamp[i] = FERSunpack32(index, data);
		index += 4;
	}
	for (int i = 0; i < nchan; ++i) {
		tmpEvent.ToT[i] = FERSunpack16(index, data);
		index += 2;
	}
	return tmpEvent;
}
