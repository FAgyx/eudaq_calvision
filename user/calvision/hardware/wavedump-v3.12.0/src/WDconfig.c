...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
		...
		if (strstr(str, "OPEN") != NULL) {
			...
			else if (strcmp(str1, "USB_A4818") == 0 || strcmp(str1, "USB_A4818_V2718") == 0 || strcmp(str1, "USB_A4818_V3718") == 0 || strcmp(str1, "USB_A4818_V4718") == 0)
			...
			if (WDcfg->LinkType == CAEN_DGTZ_ETH_V4718)
				read = fscanf(f_ini, "%s", &WDcfg->HostName);
			else
				read = fscanf(f_ini, "%d", &WDcfg->LinkNum);
			if (WDcfg->LinkType == CAEN_DGTZ_USB || WDcfg->LinkType == CAEN_DGTZ_USB_V4718 || WDcfg->LinkType == CAEN_DGTZ_ETH_V4718)
				WDcfg->ConetNode = 0;
			else
				read = fscanf(f_ini, "%d", &WDcfg->ConetNode);
			read = fscanf(f_ini, "%"SCNx32, &WDcfg->BaseAddress);
			continue;
		}

		// Generic VME Write (address offset + data + mask, each exadecimal)
		if ((strstr(str, "WRITE_REGISTER")!=NULL) && (WDcfg->GWn < MAX_GW)) {
			read = fscanf(f_ini, "%"SCNx32, &WDcfg->GWaddr[WDcfg->GWn]);
			read = fscanf(f_ini, "%"SCNx32, &WDcfg->GWdata[WDcfg->GWn]);
            read = fscanf(f_ini, "%"SCNx32, &WDcfg->GWmask[WDcfg->GWn]);
			WDcfg->GWn++;
			continue;
		}
		...
		if (strstr(str, "RECORD_LENGTH")!=NULL) {
			read = fscanf(f_ini, "%"SCNu32, &WDcfg->RecordLength);
			continue;
		}
		...
		if (strstr(str, "DECIMATION_FACTOR")!=NULL) {
			read = fscanf(f_ini, "%"SCNd16, &WDcfg->DecimationFactor);
			continue;
		}
