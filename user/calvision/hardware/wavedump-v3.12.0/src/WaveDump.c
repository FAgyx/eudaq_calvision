...
#define WaveDump_Release        "3.12.0"
#define WaveDump_Release_Date   "June 2025"
#define DBG_TIME
...
int32_t Set_relative_Threshold(int handle, WaveDumpConfig_t* WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo) {
    ...
    ret |= CAEN_DGTZ_AllocateEvent(handle, &evtbuff_p);
    if (ret != CAEN_DGTZ_Success) {
        ret = ERR_MALLOC;
        goto QuitFunction;
    }
...
QuitFunction:
    if (evtbuff_p != NULL)
        CAEN_DGTZ_FreeEvent(handle, &evtbuff_p);
    if (buffer != NULL)
        CAEN_DGTZ_FreeReadoutBuffer(&buffer);
    ...
    if ((c < '9') && (c >= '0')) {
        ...
        if ((BoardInfo.FamilyCode == CAEN_DGTZ_XX740_FAMILY_CODE)) {
            ...
            if (c > '8')
                ...
                if ((WDcfg->FastTriggerEnabled == 0) && (ch == 8))
        ...
        case 'g' :
            if (WDcfg->Nch <= 8)
                printf("Channel group can be changed only for boards with > 8 channels\n");
			//for boards with >8 channels
			if ((BoardInfo.FamilyCode == CAEN_DGTZ_XX730_FAMILY_CODE) || (BoardInfo.FamilyCode == CAEN_DGTZ_XX725_FAMILY_CODE))
			{
				if (WDrun->GroupPlotSwitch == 0) {
                    if ((WDcfg->EnableMask & 0xFF00) == 0) {
                        printf("No channel in group 1 is enabled for acquisition\n");
                    }
                    else {
                    ...
                    if ((WDcfg->EnableMask & 0xFF) == 0) {
                        printf("No channel in group 0 is enabled for acquisition\n");
                    }
                    else {
                ...
                int enabled_groups = 0;
                for (int g = 0; g < MAX_GROUPS; g++) {
                    if (WDcfg->EnableMask & (1 << g))
                        enabled_groups++;
                }
                // Update the group plot index if at least 2 groups are enabled
                if (enabled_groups > 1)
                    GoToNextEnabledGroup(WDrun, WDcfg);
                else
                    printf("Only one channel group is enabled, cannot switch group.\n");
            }
            break;
        case 'q' :
            WDrun->Quit = 1;
...
int main(int argc, char *argv[])
{
    ...
    if (WDcfg.LinkType == CAEN_DGTZ_ETH_V4718) {
        ...
        linkNum = WDcfg.LinkNum;
        arg = &linkNum;
    }

    ret = CAEN_DGTZ_OpenDigitizer2(WDcfg.LinkType, arg, WDcfg.ConetNode, WDcfg.BaseAddress, &handle);
    if (ret) {
...
Restart:
    ...
    if ((BoardInfo.FamilyCode != CAEN_DGTZ_XX740_FAMILY_CODE) && (BoardInfo.FamilyCode != CAEN_DGTZ_XX742_FAMILY_CODE)){
        ...
        if (WDcfg.EnableMask)
            WDrun.ChannelPlotMask = 0xFF;
        else
            WDrun.ChannelPlotMask = 0;
        
        if((BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) && WDcfg.FastTriggerEnabled )
            WDrun.ChannelPlotMask |= 0x100;
    }
            ...
            if (WDcfg.Nbit == 8) 
                ...
                if (WDrun.RunHisto) {
                    for(ch=0; ch<WDcfg.Nch; ch++) {
                        ...
                        if (WDcfg.Nbit == 8)
                            for(int j=0; j<(int)Event8->ChSize[ch]; j++)
                                WDrun.Histogram[ch][Event8->DataChannel[ch][j]]++;
                        else {
                            if (BoardInfo.FamilyCode != CAEN_DGTZ_XX742_FAMILY_CODE) {
                                for(int j=0; j<(int)Event16->ChSize[ch]; j++)
                                    WDrun.Histogram[ch][Event16->DataChannel[ch][j]]++;
                            }
                            else {
